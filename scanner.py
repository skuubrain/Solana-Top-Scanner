...
def generate_scan():
    ...
    # Fetch trades per wallet
    token_to_wallets = defaultdict(set)
    for i, w in enumerate(wallets):
        print(f"🔍 Scanning wallet {i+1}/{len(wallets)}: {w}")
        trades_response = do_request(f"/wallet/{w}/trades", {"limit": 200})
        ...
        for tr in trades:
            ...
            if token:
                token_to_wallets[token].add(w)
        time.sleep(REQUEST_DELAY)

    # Filter tokens
    candidates = [{"token": tok, "wallets": list(ws), "count": len(ws)}
                  for tok, ws in token_to_wallets.items() if len(ws) >= MIN_WALLETS_FOR_SIGNAL]
    ...
    print(f"✅ Done! {len(candidates)} tokens found. Files saved.")
    return candidates
